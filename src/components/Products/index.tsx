    ...
    <>
            ...
            {products.map(item => (
              <div key={item.id} className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <div className="h-56 w-full">
                  <Link to={`/${ROUTES.GET_PRODUCT_DETAIL_ROUTE(item.id)}`}>
                    <img className="mx-auto h-full dark:hidden" src={item.imageLight} alt="" />
                    <img className="mx-auto hidden h-full dark:block" src={item.imageDark} alt="" />
                  </Link>
                </div>
                <div className="pt-6">
                  <div className="mb-4 flex products-center justify-between gap-4">
                    ...
                    <div className="flex products-center justify-end gap-1">
                      <button 
                        ...
                        onClick={() => navigate(`/${ROUTES.GET_PRODUCT_DETAIL_ROUTE(item.id)}`)}
                      >
      ...
      {products.length === 0 && !isLoading && (
        <div
          ...
          role="alert"
        >
          <span className="font-medium">No hay productos disponibles!</span> para el
          día de hoy.
        </div>
      )}
